<%@ page language="java" contentType="text/html; charset=UTF-8"
pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    <title>笔趣阁</title>

    <meta name="keywords" content="笔趣阁,笔趣看">
    <meta name="description" content="">

    <meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1">
    <meta name="renderer" content="webkit">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="applicable-device" content="pc,mobile">
    <meta http-equiv="Cache-Control" content="no-transform">
    <meta http-equiv="Cache-Control" content="no-siteapp">

    <link href="https://www.biqukan.cc/novel/css/bootstrap.min.css" rel="stylesheet">
    <link href="https://www.biqukan.cc/novel/css/font-awesome.min.css" rel="stylesheet">
    <link href="https://www.biqukan.cc/novel/css/site.css" rel="stylesheet">
    <link rel="apple-touch-icon-precomposed" href="https://www.biqukan.cc/novel/images/apple-touch-icon-precomposed.png">
    <meta name="apple-mobile-web-app-title" content="笔趣阁">



    <!--[if lt IE 9]><script src="/novel/js/respond.min.js"></script><![endif]-->


</head>
<body>
<div class="navbar navbar-inverse" id="header">
    <div class="container">
        <div class="navbar-header">
            <button class="navbar-toggle collapsed" type="button" data-toggle="collapse" data-target=".bs-navbar-collapse">
                <span class="icon-bar"></span>
                <span class="icon-bar"></span>
                <span class="icon-bar"></span>
            </button>
            <a href="https://www.biqukan.cc/" class="navbar-brand logo" title="笔趣阁">笔趣阁</a>
        </div>
        <nav class="collapse navbar-collapse bs-navbar-collapse" role="navigation" id="nav-header">
            <ul class="nav navbar-nav">
                <li><a href="#" title="玄幻魔法">玄幻魔法</a></li>
                <li><a href="#" title="武侠修真">武侠修真</a></li>
                <li><a href="#" title="都市言情">都市言情</a></li>
                <li><a href="#" title="历史军事">历史军事</a></li>
                <li><a href="#" title="侦探推理">侦探推理</a></li>
                <li><a href="#" title="网游动漫">网游动漫</a></li>
                <li class="dropdown">
                    <a class="dropdown-toggle" data-toggle="dropdown">更多分类<span class="caret"></span></a>
                    <ul class="dropdown-menu" role="menu">
                        <li><a href="#" title="科幻小说">科幻小说</a></li>
                        <li><a href="#" title="恐怖灵异">恐怖灵异</a></li>
                        <li><a href="#" title="穿越小说">穿越小说</a></li>
                        <li><a href="#" title="其他类型">其他类型</a></li>
                    </ul>
                </li>
                <li class="dropdown">
                    <a class="dropdown-toggle" data-toggle="dropdown">排行榜<span class="caret"></span></a>
                    <ul class="dropdown-menu" role="menu">
                        <li><a href="#" title="点击总榜">点击总榜</a></li>
                        <li><a href="#" title="推荐总榜">推荐总榜</a></li>
                        <li><a href="#" title="点击月榜">点击月榜</a></li>
                        <li><a href="#" title="推荐月榜">推荐月榜</a></li>
                        <li><a href="#" title="周排行榜">周排行榜</a></li>
                        <li><a href="#" title="周推荐榜">周推荐榜</a></li>
                        <li><a href="#" title="总收藏榜">总收藏榜</a></li>
                        <li><a href="#" title="本站推荐">本站推荐</a></li>
                        <li><a href="#" title="字数排行">字数排行</a></li>
                        <li><a href="#" title="最近更新">最近更新</a></li>
                        <li><a href="#" title="最新入库">最新入库</a></li>
                    </ul>
                </li>
                <li><a href="#" title="完本小说">完本小说</a></li>
            </ul>
            <ul class="nav navbar-nav navbar-right">
                <li><a href="#">登录</a></li><li><a href="#">免费注册</a></li>

            </ul>
        </nav>
    </div>
</div>
<div class="container body-content">

    <div class="panel panel-default">
        <div class="panel-heading"><h1 class="fs-18 mg0"><strong>搜索结果</strong></h1></div>
        <table class="table">
            <tbody>

            <tr align="center">
                <th width="20%">文章名称</th>
                <th width="40%" class="hidden-xs">最新章节</th>
                <th width="15%">作者</th>
                <th width="9%" class="hidden-xs">字数</th>
                <th width="10%" class="hidden-xs">更新</th>
                <th width="6%">状态</th>
            </tr>

            <tr>
                <td class="odd"><a href="#">斗罗大陆4终极斗罗</a></td>
                <td class="hidden-xs"><a href="#" target="_blank"> 第二百九十四章 樱落红</a></td>
                <td class="odd">唐家三少</td>
                <td class="hidden-xs">1568K</td>
                <td class="hidden-xs">19-05-08</td>
                <td class="even">连载中</td>
            </tr>

            </tbody>
        </table>

    </div>

    <ul class="pagination" id="pagelink">
        <li class="disabled">
            <span>1/1</span>
        </li>
        <li>
            <a href="#" class="first">1</a>
        </li>
        <li>
            <a href="#" class="pgroup">&lt;&lt;</a>
        </li>
        <li class="active"><span>1</span></li><li><a href="#" class="ngroup">&gt;&gt;</a>
    </li>
        <li><a href="#" class="last">1</a>
        </li>
    </ul>

    <footer>
        <p>本站所有小说为转载作品，所有章节均由网友上传，转载至本站只是为了宣传本书让更多读者欣赏，如有侵权请 <a href="#" style="text-decoration: underline;">联系我们</a>，我们会尽快处理。</p>
        <p>Copyright &#169; 2019 <a href="#">笔趣阁</a>(www.biqukan.cc) All Rights Reserved. 沪ICP备19819998号</p>
    </footer>

    <div class="back-to-top" id="back-to-top" title="返回顶部" style="left: 1254.5px;"><i class="fa fa-chevron-up"></i></div>

</div>

</body></html>